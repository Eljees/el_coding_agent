...
def test_ask_rag_includes_retrieved_context(monkeypatch, tmp_path: Path, capsys) -> None:
    ...
    def fake_loader(root, cfg, query):  # noqa: ANN001
        return "### local_codex_lite/patcher.py [1-20] (score=3.0; matched: patch)\npatch repair context"

    def fake_client(*args, **kwargs):  # noqa: ANN001, ANN003
        client = _DummyClient()
        captured["client"] = client
        return client

    monkeypatch.setattr(cli, "workspace_root", lambda: tmp_path)
    monkeypatch.setattr(cli, "_load_rag_context", fake_loader)
    monkeypatch.setattr(cli, "OpenAICompatibleClient", fake_client)

    args = argparse.Namespace(evidence_file=[], evidence_stdin=False, rag=True)
    result = cli.cmd_ask("how does patch repair work?", args)
...
def test_preview_rag_passes_retrieved_context_into_prompts(monkeypatch, tmp_path: Path, capsys) -> None:
    captured = {}

    def fake_loader(root, cfg, query):  # noqa: ANN001
        return "### local_codex_lite/logging_utils.py [10-20] (score=2.0; matched: logs)\nlogging context"

    def fake_make_plan(task, root, cfg, run_dir=None, extra_context="", runtime_fix=None):  # noqa: ANN001
    ...
    def fake_make_patch(task, plan, root, cfg, run_dir=None, extra_context="", runtime_fix=None):  # noqa: ANN001
    ...
    monkeypatch.setattr(cli, "_selected_files", lambda root, task, cfg: [])
    monkeypatch.setattr(cli, "_load_rag_context", fake_loader)
    monkeypatch.setattr(cli, "make_plan", fake_make_plan)
    monkeypatch.setattr(cli, "make_patch", fake_make_patch)
    monkeypatch.setattr(cli, "preview_patch", lambda root, diff: 0)

    args = argparse.Namespace(task="improve logs latest", evidence_file=[], evidence_stdin=False, rag=True)
    result = cli.cmd_preview(args)
